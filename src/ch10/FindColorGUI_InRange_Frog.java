...
public class FindColorGUI_InRange_Frog {
	...
	private static final String FILE_PATH = "E:\\workspace_OpenCV\\OpenCV_3.1\\images\\blueCircle.png";
	private Mat imgsrc;
	...
	private JLabel lblMixedHSV;

	private JSlider sliderKSize;
	private JTextField textFieldKsize;
	private int KSize = 1; // for gaussian

	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FindColorGUI_InRange_Frog window = new FindColorGUI_InRange_Frog();
	...
	public FindColorGUI_InRange_Frog() {
...
//		System.out.println("000 >>> " + colorMat.dump());
	...
	protected Mat findContours(Mat mat) {
		...
		for (int i = 0 ; i < contours.size() ; i++) {
			...
			// 找質心 [ http://monkeycoding.com/?p=617 ]
			MatOfPoint contourMat = contours.get(i);
			Moments mu = Imgproc.moments(contourMat, false);
			System.out.println("momentOfInteria >>> " + mu);
			Point center = new Point(mu.m10 / mu.m00, mu.m01 / mu.m00);
			Imgproc.circle(imgsrcClone, center, 2, new Scalar(0, 0, 255), 2);
		}
