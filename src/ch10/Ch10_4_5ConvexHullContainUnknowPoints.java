...
public class Ch10_4_5ConvexHullContainUnknowPoints {
	...
	public static void main(String[] args) {
		Mat img = Mat.zeros(250*2, 250*2, CvType.CV_8UC3);
		...
		int X, Y;
		for (int i = 0 ; i < 20 ; i++) {
			X = (int) (Math.random() * 200*2 + 40);
			Y = (int) (Math.random() * 200*2 + 40);
			Imgproc.circle(img, new Point(X, Y), 2, new Scalar(0, 0, 255));
		...
		int index = (int) hull.get(((int) hull.size().height) - 1, 0)[0];//※※※設定起始點為陣列最末元素
		...
		//Imgcodecs.imwrite("C://opencv3.1//samples//rectContainPoint-convexHull.jpg", img);
		Imshow.show(img);
	}
