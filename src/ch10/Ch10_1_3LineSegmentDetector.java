...
public class Ch10_1_3LineSegmentDetector {
	...
	private void initialize() {
		final Mat source = Imgcodecs.imread(FILEPATH_01);
		...
		slider_quant.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent arg0) {
				...
				BufferedImage newImage=matToBufferedImage(LineSegmentDetector(slider_refine.getValue(),(float)slider_scale.getValue()/10,(float)slider_sigma_scale.getValue()/10,(float)slider_quant.getValue()/10,(float)slider_AngTh.getValue()/10,slider_log_eps.getValue(),(float)slider_density_th.getValue()/10,slider_n_bits.getValue()));
	...
	public Mat LineSegmentDetector(int refine, float scale, float sigma_scale, float quant,float ang_th,int log_eps, float density_th, int n_bits){
		Mat source =Imgcodecs.imread(FILEPATH_01,Imgcodecs.CV_LOAD_IMAGE_GRAYSCALE);
			Mat Line=new Mat();
			Mat draw=new Mat(source.size(),source.type());
			org.opencv.imgproc.LineSegmentDetector lsd=Imgproc.createLineSegmentDetector(refine, scale, sigma_scale, quant, ang_th, log_eps,  density_th,  n_bits);
